-- Local parameters
local StartBmxRace = 5.0              -- distance to prompt to start race
local BmxRaceHud = 5.0                -- distance to start rendering the race name text -- DRAW_TEXT_DISTANCE
local BmxRaceScores = 5.0               -- Distance to start rendering the race scores -- DRAW_SCORES_DISTANCE
local BmxMaxScores = 10                -- Maximum number of scores to draw above race title -- DRAW_SCORES_COUNT_MAX
local BmxZAxis = -5.00               -- checkpoint offset in z-axis -- CHECKPOINT_Z_OFFSET
local BmxHudColor = {238, 198, 78, 255}    -- color for racing HUD above map -- RACING_HUD_COLOR
    ...
    while raceState.index == 0 do
        ...
        for index, race in pairs(races) do
            if race.isEnabled then
                ...
                if GetDistanceBetweenCoords( race.start.x, race.start.y, race.start.z, GetEntityCoords(player)) < BmxRaceHud then
                    -- Draw race name
                    Draw3DText(race.start.x, race.start.y, race.start.z-0.600, race.title, BmxHudColor, 4, 0.3, 0.3)
                end

                -- When close enough, draw scores
                if GetDistanceBetweenCoords( race.start.x, race.start.y, race.start.z, GetEntityCoords(player)) < BmxRaceScores then
                ...
                if GetDistanceBetweenCoords( race.start.x, race.start.y, race.start.z, GetEntityCoords(player)) < StartBmxRace then
                    local vehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)
                    local mph = 2.236936
                    local speed = GetEntitySpeed(vehicle) * mph
                    if speed > Config.AllowedSpeed then
                    helpMessage("Stop moving to start time trials")
                else
                    helpMessage("Press ~INPUT_CONTEXT~ to Race!, L to Cancel")
...
AddEventHandler("raceCountdown", function()
    ...
    Citizen.CreateThread(function()
        ...
        EnableControlAction(2, 137, true)
...
AddEventHandler("raceRaceActive", function()
    ...
    Citizen.CreateThread(function()
        ...
        checkpoint = CreateCheckpoint(race.checkpoints[raceState.cP].type, race.checkpoints[raceState.cP].x,  race.checkpoints[raceState.cP].y,  race.checkpoints[raceState.cP].z + BmxZAxis, race.checkpoints[raceState.cP].x,race.checkpoints[raceState.cP].y, race.checkpoints[raceState.cP].z, race.checkpointRadius, 50, 50, 1, math.ceil(50*race.checkpointTransparency), 0)
        ...
        while raceState.index ~= 0 do 
            ...
            DrawHudText(("%.3fs"):format((GetGameTimer() - raceState.startTime)/1000), BmxHudColor, 0.015, 0.725, 0.7, 0.7)
            DrawHudText(string.format("Checkpoint %i / %i (%d m)", raceState.cP, #race.checkpoints, checkpointDist), BmxHudColor, 0.015, 0.765, 0.5, 0.5)
